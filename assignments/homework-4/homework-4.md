...
int BinarySearch(int* data, int key, int size) {
	...
	while (!found) {
		if (data[middle] == key) {
			...
			cout << "The current key doesn't exist in the array" << endl;
			break; //breaking out of the loop will prevent the infinite loop
			//that would've happened since !found would continue to be true
			//while the key isn't present in the array.
		}
		else {
			if (key < data[middle]) {
				right = middle;
