/*
@ProgramName: Infix->Postfix Calculator
@Author: Jackson Tran
@Description: This program will modify infix evaluations to postfix
evaluations and then it will calculate the postfix evaluation
...
* @Description:
*    Checks the precedence in which operations will be performed
* @Params:
*    char c - Will be evaluated to see the priority of the current operator
* @Returns:
...
/*
Expressions will be read from an input file in main. The expression will then
be converted from an infix expression to a postfix expression. The postfix
expresssion will be calculated to get an answer.
*/
int main() {
	ifstream infile;
	infile.open("exp.txt");
	...
	srand(546365);
	string infix; //The string the expression will be stored in
	double x, y;//Holds values popped off the stack to be evaluated
	char ch;//Holds a character popped off from the queue
	double eval = 0;//The result of x and y evaluated
	double result = 0; //Result of the infix/postfix evaluations
	int numExps; //Number of expressions from input file
	infile >> numExps;
